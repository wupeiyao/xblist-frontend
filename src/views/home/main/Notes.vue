...
const getTaskList = async () => {
  ...
  if (result.code === 200) {
    ...
    // 筛选出 isImportant 为 1 的任务
    TaskArr.value = result.data.records
      .filter((task) => task.isImportant === 1)
      .map((task) => {
        const category = fillCategory.value.find(
          (cat) => Number(cat.id) === task.categoryId
        );
        return {
          ...task,
          categoryName: category ? category.cname : '未知类别', // 添加类别名称
        };
      });
  }
};
