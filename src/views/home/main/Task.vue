...
const getTaskList = async () => {
  ...
  if (result.code == 200) {
    ...
    TaskArr.value = result.data.records.map((task) => {
      const category = fillCategory.value.find((cat) => Number(cat.id) === task.categoryId);
      return {
        ...task,
        categoryName: category ? category.cname : '未知类别',
      };
    });
  }
};
...
const onConfirmC = ({ selectedOptions }: any) => {
  ...
  if (selectedOptions.length > 0) {
    fieldValue.value = selectedOptions[0].text; // 选择的类别名称
...
const showTaskDetail = (task: any) => {
  ...
  formData.value = { // 填充当前任务的详情
    ...
    categoryName: category ? category.cname : '未知类别', // 新增类别名称
  };
};
...
<template>
  ...
  <div class="all">
    <div style="width: 100%;height: 7vh;margin-bottom: 7px;display: flex;justify-content: center" class="van-search">
      <van-search style="min-width: 80vw" placeholder="请输入搜索关键词" v-model="searchQuery" @search="getTaskList" />
    </div>
    ...
    <div class="task">
      <van-row v-for="task in TaskArr" class="card" style="margin-bottom: 8px;">
        <van-col :span="5" style="display: flex; justify-content: center; align-items: center">
          <van-icon name="checked" size="30px" :color="task.status === 1 ? 'green' : '#d7e0f5'"
            @click="changeStatus(task.id, task.status)" />
        </van-col>
        <van-col :span="16" @click="showTaskDetail(task)">
          <p style="margin-top: 20px">{{ task.taskTitle }}</p>
        </van-col>
        <van-col :span="3" size="27px" style="display: flex; justify-content: center; align-items: center">
          <van-icon name="star" :color="task.isImportant ? '#9480ff' : '#d7e0f5'"
            @click="changeImportant(task.id, task.isImportant)" />
        </van-col>
      </van-row>
    </div>
    <van-popup v-model:show="popupVisible" position="bottom">
      <van-form @submit="onSubmit">
        ...
        <van-field v-model="formData.categoryName" is-link readonly label="任务类别" placeholder="选择任务类别"
    ...
    <van-popup v-model:show="showPicker" round position="bottom">
      <van-picker :columns="columns" @cancel="showPicker = false" @confirm="onConfirmC" />
    </van-popup>
  </div>
